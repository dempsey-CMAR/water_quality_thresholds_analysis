...
for(i in 1:nrow(depl_fresh)){
  ...
  cat('\n###', 
    ...
    dat_i <-  dat %>%
      ...
      p <- ggplot(
        dat_i,
        aes(timestamp_utc, value, colour = factor(depth_log))
      ) +
        geom_point() +
...
- Looks like Hourglass Lake sensor malfunctioned.
...
p <- dat %>% 
  ...
  ggplot(aes(timestamp_utc, value, col = factor(depth_log))) +
  geom_point() +
  scale_colour_viridis_d(direction = -1) +
...
depth_stats <- depth_stats %>% 
  filter(
    !(station == "Long Beach" & deployment_range == "2020-Jul-16 to 2020-Nov-29" & depth_log == 5),
    !(station == "Sandy Cove" & deployment_range == "2020-Jul-16 to 2020-Nov-30" & depth_log == 5),
    !(station == "Tickle Island 1" & deployment_range == "2020-Oct-21 to 2021-Aug-25" & depth_log == 5)
  )
...
p <- ggplot(
  depth_stats, 
  aes(
    depth_log, min, col = factor(depth_log),
    text = paste0(
      "depth_log: ", depth_log, "\n",
      "min_measured: ", min, "\n",
      "station: ", station, "\n",
