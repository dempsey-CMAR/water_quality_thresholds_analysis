...
spike_table <- params$quartiles %>% 
  select(county, variable, sensor_type, all_of(thresh)) %>%
...
for(i in seq_along(counties)) {
  ...
  spike_table_i <- filter(spike_table, county == county_i)
...
qc_summary <- dat_qc %>%
  rename(spike_flag = spike_flag_sensor_depth_measured_m) %>%
...
for(i in seq_along(stations)){
  ...
  for(j in 1:length(deployments)){
    ...
    p <- p$sensor_depth_measured_m$spike
