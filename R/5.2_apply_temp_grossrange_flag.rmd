...
thresh <- threshold_tables %>% 
  select(-season) %>% 
  filter(variable == "temperature_degree_c", qc_test == "grossrange") %>% 
  mutate(
    threshold_value = case_when(
      str_detect(threshold, "user_max") ~ user_thresh$user_max,
      str_detect(threshold, "user_min") ~ user_thresh$user_min,
      TRUE ~ threshold_value
    ),
...
qc_summary <- dat_qc %>%
  rename(grossrange_flag = grossrange_flag_temperature_degree_c) %>%
...
# `r county` QC Flags: Temperature, Grossrange
...
thresh %>% 
  select(sensor_type, threshold, threshold_value) %>% 
...
if("Suspect" %in% colnames(qc_summary)) {
  ...
  for(i in seq_along(stations)){
    ...
    for(j in 1:length(deployments)){
      ...
      #p <- p$temperature_degree_c
      
      cat('\n###', depl_j, '\n')
      
      subchunkify(p$temperature_degree_c$grossrange, fig_height = h, fig_width = 8.5)
