...
- Depth rounded to nearest whole number
  - Results in depth of 0 m for 1042 (Annapolis), Captains Pond (Antigonish), Ile due Havre (Antigonish), Sober Island (Halifax), and Back Harbour (Pictou).
- Preliminary QC was applied to the data.
  - Obvious outliers removed.
- Additional QC applied was based on review of 1.1_explore_temp_data.html
...
- Digby 
  ...
  - Monthly climatology for Digby looks reasonable (Figure 15, 16).
  - Removing these data series has a negligible impact on the distribution of observations, mean, standard deviation, and user thresholds (not shown).
- Guysborough 
...
- The *overall* mean is influenced by the number of observations each month
  - e.g., Inverness: most temperature observations from June to November (Figure 14). So the overall mean is substantially lower when calculated from the climatology (vs. from the raw observations; Figure 17).
    ...
    - Cape Breton and Colchester have more observations in the Winter, and so the overall mean is *higher* when calculated from the climatology.
...
county_month_year <- county_month_year %>% 
  mutate(year = factor(year))
...
for(i in seq_along(counties)){
  ...
  county_i <- counties[i]
  dat_i <- county_month_depth %>% filter(county == county_i)
  
  # height of figure j
  n_depth <- length(unique(dat_i$depth))
  if(n_years <= 3) h = 4
  if(n_years > 3) h = 8
