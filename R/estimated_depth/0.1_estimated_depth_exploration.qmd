...
- Measured depth is the minimum depth measured by the sensor.
...
- These deployments all look reasonable. It is likely that the sensor recorded correctly, but the depth recorded in the log is not correct.
  - Shut-In Island 2020-Nov-08 looks like it moved.
...
- Shallow sensors are more likely to have a high % difference than deep sensors.
- Shallow sensors with a high % difference do not necessarily have a high absolute difference.
...
- Use absolute difference because it is more straightforward to communicate than percent difference.
- Filter out Madeline Point 2020-Feb-26 before calculating threshold.
- Use q_95 as the threshold.
- Notes for the usage considerations:
  - Only 60 % of deployments have sensors that measure depth.
  - There are only 1 - 2 sensors per string that measure depth. The log depths for these strings will be flagged where appropriate. The depths of the other sensors on the string should be interpreted cautiously; it is likely that they are also shallower or deeper than recorded in the log.
  - Absolute difference was used to asses the log depth. In shallow areas, it would be more strict to use percent difference.
