---
title: "Salinity: Rolling Standard Deviation Thresholds Comparison"
format: 
  html:
    toc: true
    toc-depth: 3
...
dat_roll <- readRDS(here("data/sal_rolling_sd_reprocessed.rds")) %>%
  select(-c(int_sample, n_sample, rolling_sd_flag_salinity_psu)) %>%
  na.omit() %>%
  separate(col = deployment_range, into = c("depl_date", NA), sep = " to ") %>% 
  mutate(
    county_sal_roll_sd = 
      if_else(county == "Inverness", "Inverness", NA_character_)
  )

# calculate potential do thresholds from filtered data
# calculate thresholds
thresholds <- readRDS(here("data/sal_rolling_sd_prelim_qc.rds")) %>%
...
dat_summary <- dat_roll %>%
  left_join(thresholds, by = "county_sal_roll_sd") %>% 
  mutate(
...
- Additional suspected drift from biofouling was removed.
...
```{r}
thresholds %>% 
   select(county_sal_roll_sd, contains("q")) %>% 
  mutate(
    county = if_else(is.na(county_sal_roll_sd), "Other", "Inverness")
  ) %>% 
  select(-county_sal_roll_sd) %>% 
