...
rolling_sd_table <- params$quartiles %>% 
  select(county_sal_roll_sd, variable, all_of(thresh)) %>%
  rename(stdev_max = all_of(thresh)) 
attr(rolling_sd_table$stdev_max, "names") <- NULL
...
dat_qc <- dat_raw %>% 
  qc_test_rolling_sd(
    rolling_sd_table = rolling_sd_table, join_column = "county_sal_roll_sd")

# summarise flagged observations
qc_summary <- dat_qc %>%
  rename(rolling_sd_flag = rolling_sd_flag_salinity_psu) %>%
...
# QC Flags: Salinity, Rolling Standard Deviation
