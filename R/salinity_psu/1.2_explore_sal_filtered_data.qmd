...
dat <- read_csv(here("data/2_summary_filtered_data.csv"), show_col_types = FALSE) %>% 
...
- Additional QC was applied based on review of 1.1_explore_sal.html and 0.1_sal_additional_qc.html
  - Trimmed sensor drift

## Station Locations

Approximate location of stations with salinity data.
Marker size is proportional to the number of salinity observations within the county.
...
```{r}
#| fig-height: 8
#| fig-cap: The number of salinity observations in each county.
...
```{r}
#| fig-height: 8
#| fig-cap: The mean and standard deviation of salinity in each county.
#| fig-cap-location: top
...
### Table 1
```{r}
#| fig-cap: Summary statistics of salinity observations - total and by county.
#| fig-cap-location: top
...
- Inverness has the most observations because the sampling interval was 2 minutes. The sampling rate for other stations was typically 10 minutes.
- Substantial decrease in standard deviation for Digby, Guysborough, and Yarmouth compared to preliminary QC data.

## Depth
```{r}
depth <- dat %>% 
  filter(group == "all_depth", variable == "Salinity") %>% 
...
```{r}
#| fig-height: 10
#| fig-cap: Number of salinity observations at each depth.
#| fig-cap-location: top
...
```{r}
#| fig-height: 10
#| fig-cap: Mean and standard deviation of salinity at each depth.
#| fig-cap-location: top
...
- Observations at 10 m are from Inverness county, in Whycocomagh Basin (brackish).
...
```{r}
#| fig.height: 8
#| fig-cap: Depths with salinity measurements by county.
#| fig-cap-location: top
...
```{r}
#| fig-height: 10
#| fig-cap: The number of salinity obervations at each depth for each county.
#| fig-cap-location: top
...
```{r}
#| fig-height: 10
#| fig-cap: Average salinity and standard deviation for each county at each measured depth. 
...
```{r}
#| fig-height: 8
#| fig-cap: Average salinity and standard deviation of all observations (grey) and average salinity at each depth.
...
```{r}
#| fig-height: 8
#| fig-cap: The number of salinity observations in each month. 
...
### Table 2
```{r}
#| fig-cap: Summary statistics of salinity observations.
#| fig-cap-location: top
...
- The most observations are in September, October, and November because this is when the sensors were deployed in Inverness (recorded every 2 minutes).
...
```{r}
#| fig-height: 10
#| fig-cap: The number of salinity observations for each month by year.
#| fig-cap-location: top
...
- Drop in Fall 2020 is from the sensors deployed in Inverness (brackish water).


## Month + County
```{r}
month_county <- dat %>% 
  filter(group == "county_month", variable == "Salinity") %>% 
...
```{r}
#| fig-height: 12
#| fig-cap: The number of salinity observations in each month for each county.
#| fig-cap-location: top
...
- Antigonish is the only county with salinity observations for all 12 months.

## Month + County + Year
```{r}
county_month_year <- dat %>% 
  filter(group == "county_month_year", variable == "Salinity") %>% 
  group_by(year, month) %>% 
  mutate(
    n_percent = round(n * 100 / sum(n), digits = 1),
    month = month(month, label = TRUE),
...
for(i in seq_along(counties)){
  ...
  p <- plot_mean_sd_season_ribbon(
    dat_i, group = "year",
    n_group = n_years,
    facet_county = FALSE, y_limits = c(0, 35)
  )
...
- Consider whether sensors in the Bras D'Or Lakes should have separate thresholds because it is a brackish waterbody.
