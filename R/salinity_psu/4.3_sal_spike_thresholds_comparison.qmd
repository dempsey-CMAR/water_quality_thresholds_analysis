---
title: "Salinity: Spike Test Thresholds Comparison"
format: 
  html:
    toc: true
    toc-depth: 3
...
dat_raw <- readRDS(here("data/sal_rolling_sd_reprocessed.rds")) %>%
  ...
  rename(value = value_salinity_psu) %>% 
  mutate(
...
dat_summary <- dat_raw %>%
  left_join(thresholds, by = "county_sal") %>% 
  mutate(
...
- Potential thresholds are the upper quantiles (q90, q95, q99.7. and q99.9) of the pooled data.
...
```{r}
thresholds %>% 
   select(county_sal, contains("q")) %>% 
  mutate(
    county = if_else(is.na(county_sal), "Other", "Inverness")
  ) %>% 
  select(-county_sal) %>% 
