...
spike_table <- q_thresholds %>% 
  select(county_sal, spike_low = q_997) %>% 
  mutate(
...
x <- rolling_sd %>% 
  ...
  select(county, station, deployment_range, 
         rolling_sd_flag = rolling_sd_flag_salinity_psu,
         spike_flag = spike_flag_salinity_psu
         ) %>% 
  mutate(
    rolling_sd_flag = as.numeric(rolling_sd_flag),
    rolling_sd_flag = if_else(rolling_sd_flag > 1, 1, 0),
    spike_flag = if_else(spike_flag > 1, 1, 0)
  ) %>% 
  group_by(county, station, deployment_range) %>% 
  summarise(
    n_rolling = sum(rolling_sd_flag),
    n_spike = sum(spike_flag)
  )
