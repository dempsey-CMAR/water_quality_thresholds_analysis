...
dat_raw <- readRDS(here("data/do_mg_per_l_rolling_sd_reprocessed.rds")) %>%
  ...
  mutate(sensor_type = sensor_serial_number) # need sensor_type column for plot function
```
...
qc_summary <- dat_qc %>%
  rename(spike_flag = spike_flag_dissolved_oxygen_uncorrected_mg_per_l) %>%
...
# QC Flags: Dissolved Oxygen, Spike Test
...
for(i in seq_along(stations)){
  ...
  for(j in 1:length(deployments)){
    ...
    p <- p$dissolved_oxygen_uncorrected_mg_per_l$spike
