...
q_thresholds <- readRDS(here("data/do_mg_per_l_rolling_sd_prelim_qc.rds")) %>%
  select(
...
dat_raw <- readRDS(here("data/do_mg_per_l_rolling_sd_reprocessed.rds")) %>%
  select(-c(int_sample, n_sample, n_sample_effective, sd_roll, rolling_sd_flag_dissolved_oxygen_uncorrected_mg_per_l)) %>%
  rename(dissolved_oxygen_uncorrected_mg_per_l = value_dissolved_oxygen_uncorrected_mg_per_l) 
...
for(i in seq_along(counties)) {
  ...
  dat_qc[[i]] <- dat_raw %>% 
    ...
    rename(
      dissolved_oxygen_uncorrected_mg_per_l = value_dissolved_oxygen_uncorrected_mg_per_l,
      spike_flag = spike_flag_dissolved_oxygen_uncorrected_mg_per_l) %>%
    qc_test_rolling_sd()
}

dat_qc <- dat_qc %>% 
  map_df(bind_rows) %>% 
  rename(spike_flag_dissolved_oxygen_uncorrected_mg_per_l = spike_flag) %>% 
  mutate(
    sensor_type = NA,
    spike_flag_dissolved_oxygen_uncorrected_mg_per_l = as.numeric(spike_flag_dissolved_oxygen_uncorrected_mg_per_l)
  ) %>% 
  select(
    c(1:7), sensor_type,
    value_dissolved_oxygen_uncorrected_mg_per_l, 
    rolling_sd_flag_dissolved_oxygen_uncorrected_mg_per_l,
    spike_flag_dissolved_oxygen_uncorrected_mg_per_l
  )
...
```{r}
both <- dat_qc %>% 
  filter(rolling_sd_flag_dissolved_oxygen_uncorrected_mg_per_l > 2 & spike_flag_dissolved_oxygen_uncorrected_mg_per_l > 2)
