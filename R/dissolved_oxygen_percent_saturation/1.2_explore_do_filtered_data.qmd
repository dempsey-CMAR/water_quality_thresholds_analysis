...
- Depth was rounded to nearest whole number
  - Results in depth of 0 m for 1042 (Annapolis).
...
- Most Inverness observations are in the summer (June, July, August; Figure 14), when DO is typically highest.
- Antigonish has few observations in the Spring / Summer, when DO is typically highest.
...
- Dissolved oxygen sensors are typically placed at 5 m below the surface.
  - `r (depth %>% mutate(n_total = sum(n)) %>% filter(depth == 5) %>% select(n_percent))$n_percent` % of the observations are at 5 m.
  - `r sum((depth %>% mutate(n_total = sum(n)) %>% filter(depth < 5) %>% select(n_percent))$n_percent)` % of observations are shallower than 5 m.
  - `r sum((depth %>% mutate(n_total = sum(n)) %>% filter(depth > 5) %>% select(n_percent))$n_percent)` % of observations are deeper than 5 m.
...
- Try filtering to only include depths <= 6 m (includes ~ 88 % of observations from the filtered data set).
