...
The means for each grouping method were similar, but the standard deviations were not (@tbl-thresh-comp). There was limited variability among the monthly means and the county means, resulting in relatively small standard deviations. This translates into very narrow user thresholds (@fig-histogram). Preliminary quality control was applied to the data used in this analysis, and 
`r thresh_summary["month", "percent_out"]` % of these observations were still flagged using the monthly thresholds. For some months, these thresholds flag values within within one standard deviation of the monthly mean. This would lead to many "false positive" flags, i.e., observations that are flagged as [Suspect/Of Interest]{style="color: #EDA247;"}, despite being within a reasonable range. 

The standard deviation for all of the data is nearly twice as large as for the other groups. This leads to broader user thresholds and fewer false positive flags. **Correspond to literature?**. Here only `r thresh_summary["all data", "percent_out"]` % of the observations were flagged. 
