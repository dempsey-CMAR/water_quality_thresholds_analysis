...
#   here("data/summary.csv"), show_col_types = FALSE
# ) %>% 
...
#   )
# 
# # data used to generate thresholds 
# 
# 
# # station locations
...
Percent saturation describes how "full" of oxygen the water is (@eq-do-percent-sat). The maximum amount of oxygen that can be dissolved depends on the water temperature, salinity, and barometric pressure (@fig-do-solubility-sal, @fig-do-solubility-press). This theoretical maximum is called the DO solubility, and is typically calculated from equations based on those developed by Benson and Krause (1984) and Garcia and Gordon (1992)[^1] (references). The percent saturation is the *measured* DO concentration as fraction of the DO solubility (@eq-do-percent-sat). 

$$
DO_{\%saturation} = 100 * DO_{measured}/DO_{solubility}
$$ {#eq-do-percent-sat}
...
ggplot(do_sal, aes(Temperature, C_p, group = Salinity, col = Salinity)) + 
 ...
 # scale_colour_manual("Salinity (psu)", values = do_cols) +
...
Conversion between DO units is non-trivial (SCOR WG 142). DO solubility is used to convert between units of concentration and percent saturation, which means estimates of temperature, salinity, and pressure are required. For the most accurate conversion, each DO observation should have a corresponding observation of these water properties. However, this is not always feasible, particularly for long deployments. Single value estimates can be used...


- challenges with measuring
  - biofouling, sensor drift
  - different units (difficult to convert)
  - high/rapid variability
  - supersaturation
  -  Diurnal variability strongly influenced by increased photosynthesis during the day and increased(?) respiration at night
https://www.ysi.com/parameters/dissolved-oxygen
  - reasonable DO values depend on depth, oceanography, etc
...
CMAR historically measured dissolved oxygen in units of percent saturation. In 20XX, sensors that measure dissolved oxygen in units of concentration ()


### DO QC Test

QC tests for dissolved oxygen were adapted from "Manual for Real-Time Quality Control of Dissolved Oxygen Observations" [@RN26562]. These tests are listed in @tbl-do-tests.
...
Dissolved oxygen records for the threshold analysis were collected from  length(na.omit(unique(dat$station))) stations in  length(na.omit(unique(dat$county))) counties (@fig-do-station-locations). A large proportion of the records are from Guysborough county (37 %), while a very small proportion are from Cape Breton county (0.1 %) (@fig-do-n-obs).
...
#| fig-height: 7
#| fig-cap: Approximate location of stations with dissolved oxygen data. Marker size is proportional to the number of dissolved oxygen observations within the county.
...
# interactive map
leaflet(st_locations2) %>% 
  addProviderTiles(providers$CartoDB.Positron) %>% 
  addCircleMarkers(
    data = filter(st_locations2, do_units == "percent saturation"),
  ...
  addCircleMarkers(
    data = filter(st_locations2, do_units == "percent saturation & mg/L"),
    lng = ~LONGITUDE, lat = ~LATITUDE, weight = 1,
    radius = ~n_prop * 25,
    color = ~pal(county),
    fillColor =  ~pal(county),
...
- difficult to tell what is biologically relevant because the amount of DO depends on the DO solubility.
- Appropriate thresholds, eg for aquaculture, typically reported in concentration
- Some sources that say 80 to 120 %, which is similar to the "normal" range of our data
...
To give each month equal weight regardless of the number of observations collected, the user thresholds were based on the monthly climatology (it was assumed that the different number of days per month was negligible).

All temperature observations were first grouped by calendar month, and the average temperature for each month was calculated. The $avg_{Temp}$ was the average of these monthly averages, and $stdev_{Temp}$ was the standard deviation of the monthly averages (@eq-temp-avg, @eq-temp-stdev). With this approach, the mean for a given month will be weighted towards years with substantially more observations than other years for that month. Future iterations of this thresholds exercise could further standardize the data to account for this.
...
Dissolved oxygen (DO) is a measure of the amount of oxygen dissolved in water, which is a key water quality parameter for much aquatic life (Santana). There are many drivers of DO variability, including biological activity (photosynthesis, respiration, plankton blooms), tidal events, meteorology, seasonal stratification, air-sea exchange, and oxidation of organic material (Santana and QARTOD manual). 

Dissolved oxygen can be measured in different units, and is commonly expressed as concentration or percent saturation. Concentration is often reported in milligrams of gaseous oxygen per litre of water (mg / L), [which is equivalent to parts per million (ppm)]. 

Percent saturation is a measure of how "full" of oxygen the water is. The maximum amount of oxygen that can be dissolved depends on the water temperature, salinity, and pressure. This maximum amount is called the DO Solubility, and can be calculated (BK, GG) or read from a DO Solubility table (DOTables). The percent saturation is the **measured** DO concentration as fraction of the DO solubility (eqn).
* Maybe some DO solubility figures here
