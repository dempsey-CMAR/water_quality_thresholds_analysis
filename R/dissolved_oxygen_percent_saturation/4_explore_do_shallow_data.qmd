...
- Average dissolved oxygen in Antigonish is lower than other counties, BUT there is no data for July, August, and September, which typically have the highest DO (Figure 15).
...
p <- county_month %>% 
  bind_rows(
    dat %>% 
      filter(group == "all_month") %>% 
      mutate(county = "All", month = month(month, label = TRUE))
  ) %>% 
  ggplot(
    aes(month, mean, group = county, col = county)
  ) +
  geom_point(size = 3) +
...
- Colchester: data from June to December was collected in 202 and 2021; data from January to May was collected in 2022 (see Colchester figure in Climatology subsection) 
