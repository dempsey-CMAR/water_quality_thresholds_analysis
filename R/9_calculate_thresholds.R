...
# the rolling standard deviation for some variables (e.g., temperature).
...
climatology_do <- qc_calculate_climatology_thresholds(
  dat_do, var = dissolved_oxygen_percent_saturation, n_sd = 3
)
...
rolling_sd_do <- qc_calculate_rolling_sd_thresholds(
  ...
  stat = "quartile", prob = 0.95
)

# spike thresholds
spike_do <- qc_calculate_spike_thresholds(
  dat_do,
  var = dissolved_oxygen_percent_saturation,
  stat = "quartile", prob = 0.997, fail_factor = 3
)
...
climatology_do_mg_per_l <- qc_calculate_climatology_thresholds(
  dat_do_mg_per_l, var = dissolved_oxygen_uncorrected_mg_per_l, n_sd = 3
)
...
# spike thresholds
spike_do_mg_per_l <- qc_calculate_spike_thresholds(
  dat_do_mg_per_l,
  var = dissolved_oxygen_uncorrected_mg_per_l,
  stat = "quartile", prob = 0.997, fail_factor = 3
)
...
# spike thresholds (group by county and sensor type)
spike_depth <- dat_depth %>%
  qc_calculate_spike_thresholds(
    county, sensor_type,
    stat = "quartile", prob = 0.997,
    var = sensor_depth_measured_m,
...
climatology_temp <- dat_temp %>%
  ...
  qc_calculate_climatology_thresholds(var = temperature_degree_c, county) %>%
  ungroup()

# gross range user thresholds
user_temp <- dat_temp %>%
  group_by(county) %>%
  qc_calculate_user_thresholds(var = temperature_degree_c) %>%
  ungroup()
...
rolling_sd_temp <- dat_temp %>%
  group_by(county) %>%
  qc_calculate_rolling_sd_thresholds(
    var = temperature_degree_c, stat = "quartile", prob = 0.997) %>%
  ungroup()
...
thresholds <- bind_rows(
  grossrange,
  depth_crosscheck,
  climatology_do, user_do, rolling_sd_do, spike_do,
  climatology_do_mg_per_l, user_do_mg_per_l, rolling_sd_do_mg_per_l,
    spike_do_mg_per_l,
  climatology_sal, user_sal, rolling_sd_sal, spike_sal,
  climatology_depth, user_depth, rolling_sd_depth, spike_depth,
  climatology_temp, user_temp, rolling_sd_temp, spike_temp
) %>%
  select(qc_test, variable, county, month, sensor_type, threshold, threshold_value)


fwrite(thresholds, file = here("output/9_thresholds.csv"), na = "NA")
